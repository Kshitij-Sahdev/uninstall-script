...
if ($nodeResponse -eq "y" -or $nodeResponse -eq "Y") {
    Write-Host "Please uninstall Node.js manually from Windows Settings" -ForegroundColor Cyan
...
if ($chromeRemoteResponse -eq "y" -or $chromeRemoteResponse -eq "Y") {
    Write-Host "Please uninstall Chrome Remote Desktop manually from Settings and Programs" -ForegroundColor Cyan
...
if ($vboxResponse -eq "y" -or $vboxResponse -eq "Y") {
    Write-Host "Please uninstall Oracle VirtualBox 7.1.4 manually from Windows Settings" -ForegroundColor Cyan
...
if ($playstationResponse -eq "y" -or $playstationResponse -eq "Y") {
    Write-Host "Please uninstall PlayStation(R) PC SDK Runtime manually from Windows Settings" -ForegroundColor Cyan
...
if ($codetantraResponse -eq "y" -or $codetantraResponse -eq "Y") {
    Write-Host "Please uninstall CodeTantra SEA 3.0.7 manually from Windows Settings" -ForegroundColor Cyan
...
if ($wdkResponse -eq "y" -or $wdkResponse -eq "Y") {
    Write-Host "Please uninstall the following manually from Windows Settings:" -ForegroundColor Cyan
...
if ($vsInstallerResponse -eq "y" -or $vsInstallerResponse -eq "Y") {
    Write-Host "Please uninstall Microsoft Visual Studio Installer manually from Windows Settings" -ForegroundColor Cyan
...
Write-Host "Some applications may require manual removal from Windows Settings or Control Panel" -ForegroundColor Yellow
...
if ($cleanup -eq "y" -or $cleanup -eq "Y") {
    ...
    $foldersToClean = @(
        ...
        # Arc Browser        "$env:LOCALAPPDATA\Arc",
        "$env:APPDATA\Arc"
    )
